...
public class AdminDAO implements AdminDAOInterface {
	...
	public ArrayList<Admin> getAllUsers() {
		...
		ArrayList<Admin> allUsers = new ArrayList<Admin>();
		...
		try {
			...
			while (rs.next()) {
				...
				user.set_isAdmin(rs.getBoolean("is_admin"));
...
//			this.conn = doaAgent.disconnectDB(conn);
	...
	public boolean updateUser(Admin newUser) {
		String sql = "UPDATE admin set  fullName=?, username=?, email=?, password=?, address=?, DOB=?, cardNumber=?, is_admin=? WHERE id=?;";
		int count = -1;
